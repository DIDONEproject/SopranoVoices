---
title: "Gender dynamics case studies"
params:
  case_study: 1
...
# This report is rendered once per case study.
# To reproduce all case studies, run ./case_studies.sh from the repository
# root. Alternatively, execute the following block in R:
...
message("Running case study ", case_study)
...
n_tree <- 5000 # Trees in the random forest
B <- 100000 # Bootstrap replicates for model comparison
...
knitr::opts_chunk$set(
  ...
  fig.path = paste0("case_studies_files/case_study_", params$case_study, "/")
)
...
# Windowed, count/sum, note-name, and syllable columns to exclude
drop_terms <- c(
  "_win_", "Count", "Sum", "Notes", "SoundingMeasures", "PartSop_Syllables"
)

# Categorical columns excluded from the feature set
categorical_terms <- c(
...
# Names of the non-categorical soprano features considered in the study:
# the PartSop_* columns minus the windowed/count/note-name/syllable columns,
# the note-name variants of the highest/lowest notes, and the categorical
# interval descriptors.
select_sop_features <- function(df) {

  feats <- grep("^PartSop_", colnames(df), value = TRUE)
  feats <- feats[!grepl(alt(drop_terms), feats)]
  feats <- feats[!grepl("^PartSop_HighestNote($|_)|^PartSop_LowestNote($|_)",
                        feats)]
  feats[!grepl(alt(categorical_terms), feats)]
...
```{r, read-data}
# Load the single-source dataset, downloaded from Zenodo on first use. The
# literal NA values in Sex (arias with no singer record) become real NAs, and
...
# masculine/male class in every case study
...
data_md5 <- "2b495d57628d67f962e8fe44fdfcb02d"
...
# Case study subsets and target y:
...
df <- switch(
  ...
  "3" = df_all |>
    filter(!is.na(Sex)) |>
    mutate(y = Sex),
  stop("Case study ", case_study, " not supported (expected 1, 2 or 3)")
)
...
# (all arias). Independent of the current case study, so it is rendered only
# in the case_study == 1 pass.
if (case_study == 1) {
...
# Independent of the current case study, so written only in the
# case_study == 1 pass.
if (case_study == 1) {
...
# Full dataset for the final-model recipe -- not interchangeable with
# bind_rows(train, test) because row order may affect the lambda_hat selection
# in the final ridge fit.
combined <- df |>
  select(all_of(selected_features), y)
```
...
write.csv(train_bake, paste0("preprocessed_datasets/train_bake_cs",
                             case_study, ".csv"),
          row.names = FALSE)
write.csv(test_train_bake, paste0("preprocessed_datasets/test_train_bake_cs",
                                  case_study, ".csv"),
          row.names = FALSE)
write.csv(all_bake, paste0("preprocessed_datasets/all_bake_cs", case_study,
                           ".csv"),
          row.names = FALSE)
```
...
for (name in names(datasets_to_save)) {
  ...
  # Construct filename
  file_name <- paste0("results/y_hats/", tolower(name), "_", case_study,
                      ".csv")
...
dir.create("results/case_studies", showWarnings = FALSE, recursive = TRUE)
write.csv(metrics_table_combined, paste0("results/case_studies/all_results_",
                                         "case_study_", case_study, ".csv"),
          row.names = FALSE)
```
...
write.csv(ci_df_all, paste0("results/final_model/ci_df_case_study_",
                            case_study, ".csv"),
          row.names = FALSE)
...
# Save plot
dir.create("plots", showWarnings = FALSE, recursive = TRUE)
plot_name <- paste0("plots/plot_rl_", case_study, ".png")

...
```{r, read-data}
# Load the single-source dataset (built by make_sopranovoices.R). The literal
# NA values in Sex (arias with no singer record) become real NAs, and
...
# Full dataset for the final-model recipe
  select(all_of(selected_features), y)
```
